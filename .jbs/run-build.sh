...
export PROJECT_VERSION=6.1.4.Final
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=6.1.4.Final
...
if [ -z "6.1.4.Final" ]; then
  echo "Enforce version not set, recreating original tag 6.1.4"
  git tag -m 6.1.4 -a 6.1.4 || true
else
  echo "Creating tag 6.1.4.Final to match enforced version"
  git tag -m 6.1.4.Final -a 6.1.4.Final || true
fi
