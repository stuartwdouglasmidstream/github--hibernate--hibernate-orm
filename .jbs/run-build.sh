...
export PROJECT_VERSION=6.1.5-SNAPSHOT
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=6.1.5-SNAPSHOT
...
sed -i s/hibernateVersion=.\*/hibernateVersion=$PROJECT_VERSION/ gradle/version.properties
...
if [ -z "6.1.5-SNAPSHOT" ]; then
  echo "Enforce version not set, recreating original tag 6.1.5"
  git tag -m 6.1.5 -a 6.1.5 || true
else
  echo "Creating tag 6.1.5-SNAPSHOT to match enforced version"
  git tag -m 6.1.5-SNAPSHOT -a 6.1.5-SNAPSHOT || true
fi
